...
const walletAddresses = {
  ...
  wire: `Beneficiary Account Name: Purple Vintage Calligraphy
Bank Name: Thread Bank
Bank Address: 210 E Main St.
Rogersville, TN 37857
Routing Number: 064209588
Account Number: 200002392811`,
};

// Barcode Mapping (wire does NOT have a barcode)
const walletBarcodes: Record<Exclude<typeof wallets[number], "wire">, string> = {
  btc: btcBarcode,
  eth: ethBarcode,
  usdt: usdtBarcode,
};
...
const DepositsPage: React.FC = () => {
  ...
  <div className="p-6 max-w-2xl mx-auto bg-white shadow-lg rounded-xl">
    ...
    {/* Payment Dropdown */}
    <label className="block mb-2 font-semibold">Select Wallet / Payment Method</label>
    <select
      value={wallet}
      onChange={(e) => setWallet(e.target.value as typeof wallets[number] | "wire")}
    ...
    >
      ...
      <option value="wire">Domestic Wires and ACH Transfers</option>
    </select>
  ...
  <div className="max-w-2xl mx-auto bg-white rounded-lg shadow p-6">
    ...
    <div className="border rounded-lg p-5">
      <h3 className="text-lg font-semibold mb-2 uppercase">
        {wallet === "wire" ? "Domestic Wires and ACH Transfers" : wallet.toUpperCase()}
      </h3>
      ...
      <div className="bg-yellow-50 text-yellow-700 border border-yellow-200 rounded-md p-3 flex items-start mb-4">
        <span>
          ...
          worth of {wallet === "wire" ? "funds" : wallet.toUpperCase()} into the wallet below.
        </span>
      </div>

      <div className="mb-4">
        <p className="text-gray-600 mb-1">{wallet === "wire" ? "Bank Details" : "Wallet Address"}</p>
        <div className="flex items-center bg-gray-100 border rounded-md px-3 py-2">
          ...
          <button
            onClick={() => {
              navigator.clipboard.writeText(walletAddresses[wallet]);
              alert(`${wallet === "wire" ? "Bank details" : "Wallet address"} copied!`);
            }}
      ...
      {/* Barcode */}
      {wallet !== "wire" && walletBarcodes[wallet] && (
